...
The following is a mocked up placeholder...

### About Chytrid

Chytrid fungus is an infectious fungal disease that can be fatal to amphibians and has caused some species to become extinct. Talk to Katie about this intro paragraph...
...
my_dists <- tibble(
  pi = seq(0, 1, length.out = 500),
  prob = dbeta(pi, alpha, beta)
)

my_dists %>%
  ggplot() + 
  geom_area(aes(x = pi, y = prob), 
...
```{r}
my_dists <- my_dists %>%
  mutate(post_prob = choose(num_frogs, num_pos)*pi^num_pos*(1 - pi)^(num_frogs - num_pos)) %>%
  mutate(post_prob_normalized = post_prob/(sum(post_prob)*1/500))
...
```{r}
bounds <- my_dists %>%
  mutate(post_percentile = cumsum(post_prob_normalized*(1/500))) %>%
